namespace Modbus.IO
{
    ...
    internal class ModbusIpTransport : ModbusTransport
    {
        ...
        internal static async System.Threading.Tasks.Task<byte[]> ReadRequestResponse(IStreamResource streamResource)
        {
            // read header
            ...
            {
                int bRead = await streamResource.Read(mbapHeader, numBytesRead, 6 - numBytesRead).ConfigureAwait(false);
            ...
            {
                int bRead = await streamResource.Read(messageFrame, numBytesRead, frameLength - numBytesRead).ConfigureAwait(false);
        ...
        {
            return ReadRequestResponse(StreamResource).Result;
        }

        internal override IModbusMessage ReadResponse<T>()
        {
            return CreateMessageAndInitializeTransactionId<T>(ReadRequestResponse(StreamResource).Result);
        }
